    ...
    if (projected_points.size() > 1) {
        ...
        for(int i=0;i<max_iter;i++) {
            ...
            for (int j=0;j<projected_points.size();j++)
            {
                ...
                denom += weights[j] / (val > fTiny ? val : fTiny);
...
void OriginalAggHomogEstimator::perform_aggregation()
{
    ...
    for (int j=0;j<srcPoints.size();j++)
    {
        ...
        for (int i=0;i<intermediate_models.size();i++) {
            ...
            h_x[0] /= h_x[2]; h_x[1] /= h_x[2]; h_x[2] = 1;
            //! Store
        ...
        wmean_agg_points[0+6*j] = pt[0]/pt[2];  wmean_agg_points[1+6*j] = pt[1]/pt[2]; wmean_agg_points[2+6*j] = 1;
        wmean_agg_points[3+6*j] = res_wmean[0]; wmean_agg_points[4+6*j] = res_wmean[1]; wmean_agg_points[5+6*j] = 1;
        double res_wgmed[3];
        aggregate_weighted_geometric_median(projected_points, res_wmean, res_wgmed);
        wgmed_agg_points[0+6*j] = pt[0]/pt[2];  wgmed_agg_points[1+6*j] = pt[1]/pt[2]; wgmed_agg_points[2+6*j] = 1;
        wgmed_agg_points[3+6*j] = res_wgmed[0]; wgmed_agg_points[4+6*j] = res_wgmed[1]; wgmed_agg_points[5+6*j] = 1;
