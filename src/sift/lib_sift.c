...
void sift_translate_anatomy_into_standard(const struct sift_keypoints *keys, struct sift_keypoint_std* k, int *n)
{
 ...
 int sift_compute_matches(const float *i1, int w1, int h1, const float *i2, int w2, int h2,
                                            float match_threshold, struct sift_keypoints **out_k1,
                                            struct sift_keypoints **out_k2, int verbose)
{
    ...
    struct sift_keypoints **kk1 = xmalloc(6*sizeof(struct sift_keypoints*));
    struct sift_keypoints **kk2 = xmalloc(6*sizeof(struct sift_keypoints*));
    for(int i = 0; i < 6; i++){
        kk1[i] = sift_malloc_keypoints();
        kk2[i] = sift_malloc_keypoints();
    }
    // WARNING 4 scalespace structures
    struct sift_scalespace **ss1 = xmalloc(4*sizeof(struct sift_scalespace*));
    struct sift_scalespace **ss2 = xmalloc(4*sizeof(struct sift_scalespace*));

    /** Algorithm */
    if (verbose)
        printf("Extracting features from image 1...\n");
    struct sift_keypoints* keys1 = sift_anatomy(i1, w1, h1, p, ss1, kk1);
    if (verbose) {
        printf("Found %d keypoints.\n\n", (*kk1)->size);
        printf("Extracting features from image 2...\n");
    }
    struct sift_keypoints* keys2 = sift_anatomy(i2, w2, h2, p, ss2, kk2);
    if (verbose)
        printf("Found %d keypoints.\n\n", (*kk2)->size);

    *out_k1 = sift_malloc_keypoints();
    *out_k2 = sift_malloc_keypoints();
    struct sift_keypoints *out_k2B = sift_malloc_keypoints();

    if (verbose)
        printf("Matching %d with %d keypoints.\n", (*kk1)->size, (*kk2)->size);
    matching(keys1, keys2, *out_k1, *out_k2, out_k2B, match_threshold, 1);
    if (verbose)
        printf("Found %d matches.\n\n", (*out_k1)->size);

    /* memory deallocation */
    xfree(p);
    sift_free_keypoints(keys1);
    sift_free_keypoints(keys2);
    for(int i = 0; i < 6; i++){
        sift_free_keypoints(kk1[i]);
        sift_free_keypoints(kk2[i]);
    }
    xfree(kk1);
    xfree(kk2);
    for(int i = 0; i < 4; i++){
        sift_free_scalespace(ss1[i]);
        sift_free_scalespace(ss2[i]);
    }
    xfree(ss1);
    xfree(ss2);
    return (*out_k1)->size;
